  ...
  if (ast.columns) {
    ast.columns.forEach(element => {
      if (element.expr.type === 'column_ref') {
        ...
        throw new Error(`Not implement select expr type: ${element.expr.type}`)
      }
    })
  }

  // Prepare data for WHERE stamet
  const sqloperatorsTableUsedKeys = []
  function whereCheckCode (arg) {
    if (arg.type === 'column_ref') {
      ...
      if (/^queryArgsValues\[[0-9]+\]$/.test(arg.value)) {
        const index = arg.value.match(/^queryArgsValues\[([0-9]+)\]$/)[1] - 1
        const val = JSON.stringify(queryArgsValues[index])
        return `${val}`
      }
      return `"${arg.value}"`
      ...
      throw new Error(`"Not implement arg.type = "${arg.type}" in where section"` + JSON.stringify(arg))
    }
  }
  ...
  if (ast.groupby) {
    ast.groupby.forEach(element => {
      ...
      if (element.type === 'number') {
        const indexInSelect = element.value
        column = ast.columns[indexInSelect - 1]
      } else {
        throw new Error('Not implement `group by` based on not numbers keys')
      }

      if (column.expr.type !== 'column_ref') {
        throw new Error('Not implement `group by` based on not column_ref')
      }
  ...
  if (ast.orderby && ast.orderby.length) {
    ast.orderby.forEach(element => {
      ...
      if (element.expr.type === 'number') {
        ...
        throw new Error('Not implement `order by` based on not numbers keys')
      }
    ...
    orderByColumns = `table.sort(tFinal, function (a, b)
          ${orderByColumns} 
          return a['_pry_key'] > b['_pry_key'] 
      end)`
  }

  // Prepare data for LIMIT stamet
  const limit = { start: 0, limit: 0 }
